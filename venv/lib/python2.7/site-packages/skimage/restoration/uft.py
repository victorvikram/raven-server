...
def ufftn(inarray, dim=None):
    ...
    if dim is None:
        dim = inarray.ndim
    outarray = np.fft.fftn(inarray, axes=list(range(-dim, 0)))
...
def uifftn(inarray, dim=None):
    ...
    if dim is None:
        dim = inarray.ndim
    outarray = np.fft.ifftn(inarray, axes=list(range(-dim, 0)))
    return outarray * np.sqrt(np.prod(inarray.shape[-dim:]))
...
def urfftn(inarray, dim=None):
    ...
    if dim is None:
        dim = inarray.ndim
    outarray = np.fft.rfftn(inarray, axes=list(range(-dim, 0)))
    return outarray / np.sqrt(np.prod(inarray.shape[-dim:]))
...
def uirfftn(inarray, dim=None, shape=None):
    ...
    if dim is None:
        dim = inarray.ndim
    outarray = np.fft.irfftn(inarray, shape, axes=list(range(-dim, 0)))
    ...
    for axis, axis_size in enumerate(imp_resp.shape):
        if axis >= imp_resp.ndim - dim:
            irpadded = np.roll(irpadded,
                               ...
                               axis=axis)
    if is_real:
        return np.fft.rfftn(irpadded, axes=list(range(-dim, 0)))
    else:
        return np.fft.fftn(irpadded, axes=list(range(-dim, 0)))
