...
from numpy.compat import (
    asbytes, asstr, asunicode, asbytes_nested, bytes, str, str,
...
def _savez(file, args, kwds, compress, allow_pickle=True, pickle_kwargs=None):
    ...
    if sys.version_info >= (3, 6):
        ...
        for key, val in list(namedict.items()):
            fname = key + '.npy'
...
def loadtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    try:
        ...
        for i, conv in list((user_converters or {}).items()):
            if usecols:
                try:
                    i = usecols.index(i)
                except ValueError:
...
def fromregex(file, regexp, dtype, encoding=None):
    ...
    try:
        ...
        content = file.read()
        if isinstance(content, bytes) and isinstance(regexp, np.str):
            regexp = asbytes(regexp)
        elif isinstance(content, np.str) and isinstance(regexp, bytes):
            regexp = asstr(regexp)
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    * Individual values are not stripped of spaces by default.
    ...
    uc_update = []
    for (j, conv) in list(user_converters.items()):
