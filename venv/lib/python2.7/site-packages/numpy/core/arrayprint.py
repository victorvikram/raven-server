...
class FloatingFormat(object):
    ...
    def fillFormat(self, data):
        ...
        if len(finite_vals) == 0:
            ...
            frac_strs, _, exp_strs = list(zip(*(s.partition('e') for s in strs)))
            int_part, frac_part = list(zip(*(s.split('.') for s in frac_strs)))
            ...
            strs = (dragon4_positional(x, precision=self.precision,
                                       ...
                                       sign=self.sign == '+')
                    for x in finite_vals)
            int_part, frac_part = list(zip(*(s.split('.') for s in strs)))
            if self._legacy == '1.13':
