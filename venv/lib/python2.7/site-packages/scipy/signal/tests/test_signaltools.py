...
class TestConvolve(_TestConvolve):
    ...
    def test_convolve_method(self, n=100):
        types = sum([t for _, t in list(np.sctypes.items())], [])
...
def check_filtfilt_gust(b, a, shape, axis, irlen=None):
    ...
    for indx in product(*[list(range(d)) for d in out_shape]):
...
def test_choose_conv_method():
    for mode in ['valid', 'same', 'full']:
        for ndims in [1, 2]:
            ...
            assert_('fft' in list(times.keys()) and 'direct' in list(times.keys()))
