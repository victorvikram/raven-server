...
def get_node_attributes(G, name):
    ...
    'red'
    """
    return {n: d[name] for n, d in list(G.nodes.items()) if name in d}
...
def set_edge_attributes(G, values, name=None):
    ...
    if name is not None:
        ...
        if G.is_multigraph():
            for (u, v, key), d in list(values.items()):
                try:
                    G[u][v][key].update(d)
                except KeyError:
                    pass
        else:
            for (u, v), d in list(values.items()):
                try:
                    G[u][v].update(d)
                except KeyError:
                    pass
...
def nodes_with_selfloops(G):
    ...
    """
    return (n for n, nbrs in list(G.adj.items()) if n in nbrs)
...
def selfloop_edges(G, data=False, keys=False, default=None):
    ...
    if data is True:
        if G.is_multigraph():
            if keys is True:
                return ((n, n, k, d)
                        for n, nbrs in list(G.adj.items())
            ...
            return ((n, n, nbrs[n]) for n, nbrs in list(G.adj.items()) if n in nbrs)
        ...
        if G.is_multigraph():
            ...
            return ((n, n, nbrs[n].get(data, default))
                    for n, nbrs in list(G.adj.items()) if n in nbrs)
    else:
        if G.is_multigraph():
            if keys is True:
                return ((n, n, k)
            ...
            return ((n, n) for n, nbrs in list(G.adj.items()) if n in nbrs)
