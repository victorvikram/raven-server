...
class UnionAtlas(Mapping):
    ...
    def copy(self):
        result = {nbr: dd.copy() for nbr, dd in list(self._succ.items())}
        for nbr, dd in list(self._pred.items()):
            if nbr in result:
    ...
    def copy(self):
        ...
        if node_ok_shorter:
            return {u: self._atlas[u] for u in self.NODE_OK.nodes
                    if u in self._atlas}
        return {u: d for u, d in list(self._atlas.items())
                if self.NODE_OK(u)}
...
class FilterAdjacency(Mapping):   # edgedict
    ...
    def copy(self):
        try:  # check that NODE_OK has attr 'nodes'
            ...
            node_ok_shorter = False
        if node_ok_shorter:
            return {u: {v: d for v, d in list(self._atlas[u].items())
                        if self.NODE_OK(v) if self.EDGE_OK(u, v)}
                    for u in self.NODE_OK.nodes if u in self._atlas}
        return {u: {v: d for v, d in list(nbrs.items()) if self.NODE_OK(v)
                    if self.EDGE_OK(u, v)}
                for u, nbrs in list(self._atlas.items())
                if self.NODE_OK(u)}
...
class FilterMultiInner(FilterAdjacency):  # muliedge_seconddict
    ...
    def copy(self):
        try:  # check that NODE_OK has attr 'nodes'
            ...
            node_ok_shorter = False
        if node_ok_shorter:
            return {v: {k: d for k, d in list(self._atlas[v].items())
                        if self.EDGE_OK(v, k)}
                    for v in self.NODE_OK.nodes if v in self._atlas}
        return {v: {k: d for k, d in list(nbrs.items()) if self.EDGE_OK(v, k)}
                for v, nbrs in list(self._atlas.items()) if self.NODE_OK(v)}
...
class FilterMultiAdjacency(FilterAdjacency):  # multiedgedict
    ...
    def copy(self):
        ...
        if node_ok_shorter:
            ...
            return {u: {v: {k: d for k, d in list(kd.items())
                            if self.EDGE_OK(u, v, k)}
                        for v, kd in list(self._atlas[u].items()) if v in my_nodes}
                    for u in my_nodes if u in self._atlas}
        return {u: {v: {k: d for k, d in list(kd.items())
                        if self.EDGE_OK(u, v, k)}
                    for v, kd in list(nbrs.items()) if self.NODE_OK(v)}
                for u, nbrs in list(self._atlas.items()) if self.NODE_OK(u)}
