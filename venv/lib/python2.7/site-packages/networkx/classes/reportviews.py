...
class NodeDataView(Set):
    ...
    def __iter__(self):
        ...
        return ((n, dd[data] if data in dd else self._default)
                for n, dd in list(self._nodes.items()))
...
class DiDegreeView(object):
    ...
    def __getitem__(self, n):
        ...
        return sum(dd.get(weight, 1) for dd in list(succs.values())) + \
            sum(dd.get(weight, 1) for dd in list(preds.values()))
    ...
    def __iter__(self):
        ...
        if weight is None:
            ...
            for n in self._nodes:
                succs = self._succ[n]
                preds = self._pred[n]
                deg = sum(dd.get(weight, 1) for dd in list(succs.values())) \
                    + sum(dd.get(weight, 1) for dd in list(preds.values()))
                yield (n, deg)
...
class DegreeView(DiDegreeView):
    ...
    def __getitem__(self, n):
        ...
        return sum(dd.get(weight, 1) for dd in list(nbrs.values())) + \
    ...
    def __iter__(self):
        ...
        if weight is None:
            ...
            for n in self._nodes:
                nbrs = self._succ[n]
                deg = sum(dd.get(weight, 1) for dd in list(nbrs.values())) + \
                    (n in nbrs and nbrs[n].get(weight, 1))
                yield (n, deg)
...
class OutDegreeView(DiDegreeView):
    ...
    def __getitem__(self, n):
        ...
        if self._weight is None:
            return len(nbrs)
        return sum(dd.get(self._weight, 1) for dd in list(nbrs.values()))
...
class InDegreeView(DiDegreeView):
    ...
    def __iter__(self):
        ...
        if weight is None:
            ...
            for n in self._nodes:
                preds = self._pred[n]
                deg = sum(dd.get(weight, 1) for dd in list(preds.values()))
                yield (n, deg)
...
class MultiDegreeView(DiDegreeView):
    ...
    def __getitem__(self, n):
        ...
        nbrs = self._succ[n]
        if weight is None:
            return sum(len(keys) for keys in list(nbrs.values())) + \
                (n in nbrs and len(nbrs[n]))
        # edge weighted graph - degree is sum of nbr edge weights
        deg = sum(d.get(weight, 1) for key_dict in list(nbrs.values())
                  for d in list(key_dict.values()))
        if n in nbrs:
            deg += sum(d.get(weight, 1) for d in list(nbrs[n].values()))
        return deg
    ...
    def __iter__(self):
        ...
        if weight is None:
            for n in self._nodes:
                nbrs = self._succ[n]
                deg = sum(len(keys) for keys in list(nbrs.values())) + \
                    (n in nbrs and len(nbrs[n]))
                yield (n, deg)
        else:
            for n in self._nodes:
...
class DiMultiDegreeView(DiDegreeView):
    ...
    def __getitem__(self, n):
        ...
        preds = self._pred[n]
        if weight is None:
            return sum(len(keys) for keys in list(succs.values())) + \
                sum(len(keys) for keys in list(preds.values()))
        # edge weighted graph - degree is sum of nbr edge weights
        deg = sum(d.get(weight, 1) for key_dict in list(succs.values())
                  for d in list(key_dict.values())) + \
            sum(d.get(weight, 1) for key_dict in list(preds.values())
                for d in list(key_dict.values()))
        return deg
    ...
    def __iter__(self):
        ...
        if weight is None:
            for n in self._nodes:
                succs = self._succ[n]
                preds = self._pred[n]
                deg = sum(len(keys) for keys in list(succs.values())) + \
                    sum(len(keys) for keys in list(preds.values()))
                yield (n, deg)
        else:
            for n in self._nodes:
...
class InMultiDegreeView(DiDegreeView):
    ...
    def __getitem__(self, n):
        ...
        nbrs = self._pred[n]
        if weight is None:
            return sum(len(data) for data in list(nbrs.values()))
        # edge weighted graph - degree is sum of nbr edge weights
        return sum(d.get(weight, 1) for key_dict in list(nbrs.values())
                   for d in list(key_dict.values()))
...
class OutMultiEdgeDataView(OutEdgeDataView):
    ...
    def __iter__(self):
        return (self._report(n, nbr, k, dd) for n, nbrs in self._nodes_nbrs()
                for nbr, kd in list(nbrs.items()) for k, dd in list(kd.items()))
...
class MultiEdgeDataView(OutMultiEdgeDataView):
    ...
    def __iter__(self):
        seen = {}
        for n, nbrs in self._nodes_nbrs():
            for nbr, kd in list(nbrs.items()):
                if nbr not in seen:
                    for k, dd in list(kd.items()):
