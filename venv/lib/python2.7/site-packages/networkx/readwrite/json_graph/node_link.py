...
def node_link_data(G, attrs=None):
    ...
    if attrs is None:
        ...
        attrs.update({k: v for (k, v) in list(_attrs.items()) if k not in attrs})
    name = attrs['name']
    source = attrs['source']
    target = attrs['target']
    links = attrs['link']
    ...
    data = {'directed': G.is_directed(), 'multigraph': multigraph, 'graph': G.graph,
            'nodes': [dict(chain(list(G.nodes[n].items()), [(name, n)])) for n in G]}
    if multigraph:
        data[links] = [
...
def node_link_graph(data, directed=False, multigraph=True, attrs=None):
    ...
    for d in data['nodes']:
        ...
        nodedata = dict((make_str(k), v) for k, v in list(d.items()) if k != name)
