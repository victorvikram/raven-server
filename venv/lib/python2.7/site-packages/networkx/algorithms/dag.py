...
def descendants(G, source):
    ...
    des = set(n for n, d in list(nx.shortest_path_length(G, source=source).items()))
    return des - {source}
...
def ancestors(G, source):
    ...
    if not G.has_node(source):
        raise nx.NetworkXError("The node %s is not in the graph." % source)
    anc = set(n for n, d in list(nx.shortest_path_length(G, target=source).items()))
    return anc - {source}
...
def dag_longest_path(G, weight='weight', default_weight=1):
    ...
    for v in nx.topological_sort(G):
        us = [(dist[u][0] + data.get(weight, default_weight), u)
              for u, data in list(G.pred[v].items())]
