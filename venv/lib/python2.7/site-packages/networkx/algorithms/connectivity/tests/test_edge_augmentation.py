...
def _assert_solution_properties(G, aug_edges, avail_dict=None):
    ...
    unique_aug = set(map(tuple, list(map(sorted, aug_edges))))
    unique_aug = list(map(tuple, list(map(sorted, aug_edges))))
...
def _augment_and_check(G, k, avail=None, weight=None, verbose=False,
    ...
    try:
        if avail is not None:
            ...
            avail_dict = dict(list(zip(*_unpack_available_edges(avail,
                                                           weight=weight))))
        else:
            avail_dict = None
        try:
            ...
            if avail is None:
                ...
                if max_aug_k is None:
                    G_aug_all = G.copy()
                    G_aug_all.add_edges_from(list(avail_dict.keys()))
                    try:
