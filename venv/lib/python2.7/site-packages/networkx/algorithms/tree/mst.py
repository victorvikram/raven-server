...
def prim_mst_edges(G, minimum, weight='weight',
    ...
    while nodes:
        ...
        visited = [u]
        if is_multigraph:
            for v, keydict in list(G.adj[u].items()):
                for k, d in list(keydict.items()):
                    wt = d.get(weight, 1) * sign
                    if isnan(wt):
                        if ignore_nan:
                            continue
        ...
        while frontier:
            ...
            nodes.remove(v)
            if is_multigraph:
                for w, keydict in list(G.adj[v].items()):
                    if w in visited:
                        continue
                    for k2, d2 in list(keydict.items()):
                        new_weight = d2.get(weight, 1) * sign
                        push(frontier, (new_weight, next(c), v, w, k2, d2))
            else:
                for w, d2 in list(G.adj[v].items()):
                    if w in visited:
                        continue
                    new_weight = d2.get(weight, 1) * sign
...
def minimum_spanning_tree(G, weight='weight', algorithm='kruskal',
    ...
    T.add_nodes_from(list(G.nodes.items()))
