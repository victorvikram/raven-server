...
def asyn_lpa_communities(G, weight=None, seed=None):
    ...
    while cont:
        ...
        for node in nodes:
            ...
            best_labels = [label for label, freq in list(label_freq.items())
                           if freq == max_freq]
    ...
    return iter(list(groups(labels).values()))
...
def label_propagation_communities(G):
    ...
    while not _labeling_complete(labeling, G):
        ...
        for color, nodes in list(coloring.items()):
            for n in nodes:
...
def _most_frequent_labels(node, labeling, G):
    ...
    return {label for label, freq in list(freqs.items()) if freq == max_freq}
