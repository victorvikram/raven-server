...
def greedy_modularity_communities(G, weight=None):
    ...
    # Initial modularity
    partition = [[label_for_node[x] for x in c] for c in list(communities.values())]
    ...
    dq_heap = [
        MappedQueue([
            (-dq, i, j)
            for j, dq in list(dq_dict[i].items())])
        for i in range(N)]
    H = MappedQueue([
        dq_heap[i].h[0]
        for i in range(N)
    ...
    while len(H) > 1:
        ...
        i_neighbors = list(dq_dict[i].keys())
    ...
    communities = [
        ...
        for c in list(communities.values())]
