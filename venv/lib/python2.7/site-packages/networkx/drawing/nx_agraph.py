...
def from_agraph(A, create_using=None):
    ...
    for n in A.nodes():
        str_attr = {str(k): v for k, v in list(n.attr.items())}
    ...
    for e in A.edges():
        u, v = str(e[0]), str(e[1])
        attr = dict(e.attr)
        str_attr = {str(k): v for k, v in list(attr.items())}
...
def to_agraph(N):
    ...
    A.graph_attr.update((k, v) for k, v in list(N.graph.items())
    ...
    for n, nodedata in N.nodes(data=True):
        ...
        if nodedata is not None:
            a = A.get_node(n)
            a.attr.update({k: str(v) for k, v in list(nodedata.items())})

    # loop over edges
    if N.is_multigraph():
        for u, v, key, edgedata in N.edges(data=True, keys=True):
            str_edgedata = {k: str(v) for k, v in list(edgedata.items())
                            if k != 'key'}
