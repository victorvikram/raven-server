...
def joint_degree_graph(joint_degrees, seed=None):
    ...
    degree_count = {k: sum(l.values()) // k for k, l in list(joint_degrees.items()) if k > 0}
    ...
    nodeid = 0
    for degree, num_nodes in list(degree_count.items()):
        h_degree_nodelist[degree] = list(range(nodeid, nodeid + num_nodes))
