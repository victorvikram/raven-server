...
def ring_of_cliques(num_cliques, clique_size):
    ...
    for i in range(num_cliques):
        edges = itertools.combinations(list(range(i * clique_size, i * clique_size +
                                             clique_size)), 2)
        G.add_edges_from(edges)
...
def stochastic_block_model(sizes, p, nodelist=None, seed=None,
    ...
    block_range = list(range(len(sizes)))
    if directed:
        g = nx.DiGraph()
    ...
    for i, j in block_iter:
        if i == j:
            if directed:
                ...
                if selfloops:
                    edges = itertools.chain(edges, list(zip(parts[i], parts[i])))
            for e in edges:
