...
def hparams(hparam_dict=None, metric_dict=None):
    ...
    hps = [HParamInfo(name=k) for k in list(hparam_dict.keys())]
    mts = [MetricInfo(name=MetricName(tag=k)) for k in list(metric_dict.keys())]
    ...
    for k, v in list(hparam_dict.items()):
...
def custom_scalars(layout):
    ...
    for k, v in list(layout.items()):
        charts = []
        for chart_name, chart_meatadata in list(v.items()):
