...
class Module(object):
        ...
        if len(self._backward_hooks) > 0:
            ...
            while not isinstance(var, torch.Tensor):
                if isinstance(var, dict):
                    var = next((v for v in list(var.values()) if isinstance(v, torch.Tensor)))
                else:
                    var = var[0]
            grad_fn = var.grad_fn
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        local_name_params = itertools.chain(list(self._parameters.items()), list(self._buffers.items()))
        local_state = {k: v for k, v in local_name_params if v is not None}

        for name, param in list(local_state.items()):
            key = prefix + name
    ...
    def named_parameters(self, prefix='', recurse=True):
        ...
        gen = self._named_members(
            lambda module: list(module._parameters.items()),
    ...
    def named_buffers(self, prefix='', recurse=True):
        ...
        """
        gen = self._named_members(
            lambda module: list(module._buffers.items()),
            prefix=prefix, recurse=recurse)
        for elem in gen:
            yield elem
