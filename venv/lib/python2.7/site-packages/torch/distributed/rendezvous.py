...
def rendezvous(url, rank=-1, world_size=-1, **kwargs):
    ...
    if rank != -1 or world_size != -1:
        query_dict = dict(
            pair.split("=") for pair in [_f for _f in result.query.split("&") if _f]
        )
        assert (
        ...
        result = result._replace(
            query="{}".format("&".join(["{}={}".format(k, v) for k, v in list(query_dict.items())]))
        )
...
def _file_rendezvous_handler(url, **kwargs):
    ...
    query = dict(pair.split("=") for pair in [_f for _f in result.query.split("&") if _f])
