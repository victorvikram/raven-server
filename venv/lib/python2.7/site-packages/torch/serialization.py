...
def _is_path(name_or_buffer):
    return isinstance(name_or_buffer, str) or \
        (sys.version_info[0] == 2 and isinstance(name_or_buffer, str)) or \
...
def load(f, map_location=None, pickle_module=pickle, **pickle_load_args):
    ...
    if sys.version_info >= (3, 0) and 'encoding' not in list(pickle_load_args.keys()):
...
def _get_layout(name):
    ...
    if not cache:
        for v in list(torch.__dict__.values()):
