...
def iter_indices(tensor):
    if tensor.dim() == 0:
        return list(range(0))
    if tensor.dim() == 1:
        return list(range(tensor.size(0)))
    return product(*(list(range(s)) for s in tensor.size()))
...
class TestCase(expecttest.TestCase):
    ...
    def assertEqual(self, x, y, prec=None, message='', allow_inf=False, exact_dtype=None):
        ...
        if isinstance(x, torch.Tensor) and isinstance(y, Number):
            ...
            if isinstance(x, OrderedDict) and isinstance(y, OrderedDict):
                self.assertEqual(list(x.items()), list(y.items()), prec=prec,
                                 message=message, allow_inf=allow_inf,
                                 exact_dtype=exact_dtype)
            else:
    ...
    def _reset_warning_registry(self):
        ...
        for name, reg_orig in list(backup.items()):
            try:
