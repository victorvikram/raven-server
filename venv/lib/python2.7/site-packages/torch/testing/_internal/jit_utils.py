...
class JitTestCase(TestCase):
    ...
    def _compared_saved_loaded(self, m):
        ...
        def extract_files(buffer):
            ...
            files = list([x for x in archive.namelist() if x.startswith('archive/code/')])
            # unwrap all the code files into strings
            code_files = [x for x in files if x.endswith('.py')]
            code_files = [archive.open(f) for f in code_files]
            code_files = ["".join([line.decode() for line in file]) for file in code_files]

            # unpickled all the debug files
            debug_files = [f for f in files if f.endswith('.debug_pkl')]
...
def attrs_with_prefix(module, prefix):
    return [x for x, _ in list(module._modules._c.items())
