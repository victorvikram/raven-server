...
def _trace_and_get_graph_from_model(model, args, training):
    ...
    orig_state_dict_keys = list(_unique_state_dict(model).keys())
    ...
    if orig_state_dict_keys != list(_unique_state_dict(model).keys()):
...
def _model_to_graph(model, args, verbose=False, training=False,
    ...
    params_dict = dict(list(zip(param_names, params)))
    ...
    try:
        ...
        if export_type == ExportTypes.PROTOBUF_FILE:
            ...
            with zipfile.ZipFile(f, 'w', compression=compression) as z:
                ...
                for k, v in list(export_map.items()):
                    z.writestr(k, v)
...
def _validate_dynamic_axes(dynamic_axes, model, input_names, output_names):
    ...
    for key, value in list(dynamic_axes.items()):
