    ...
    for x_tensor, d_tensor in zip(x_tensors, j_tensors):
        if x_tensor.is_sparse:
            def get_stride(size):
                ...
                tmp = 1
                stride = [0] * dim
                for i in reversed(list(range(dim))):
                    stride[i] = tmp
                    tmp *= size[i]
                return stride
            ...
            for i in range(x_nnz):
                x_value = x_values[i]
                for x_idx in product(*[list(range(m)) for m in x_values.size()[1:]]):
            ...
            if len(input) != 1:
                raise ValueError('gradcheck currently only supports functions with 1 input, but got: ',
                                 len(input))
            for d_idx, x_idx in enumerate(product(*[list(range(m)) for m in x_tensor.size()])):
