...
def _gen_unsupported_methods_properties():
    tensor_attrs = set([x for x in dir(torch.Tensor) if x[0] != "_"])
    ...
    methods = ["\t*  :meth:`~torch.Tensor." + x + r"`" for x in methods]
    properties = ["\t*  :attr:`~torch.Tensor." + x + r"`" for x in properties]
    return "\n".join(methods), "\n".join(properties)
