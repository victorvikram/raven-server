...
def get_type_line(source):
    ...
    type_lines = list([line for line in lines if type_comment in line[1]])
    lines_with_type = list([line for line in lines if 'type' in line[1]])

    if len(type_lines) == 0:
        type_pattern = re.compile('#[\t ]*type[\t ]*:')
        wrong_type_lines = list([line for line in lines if type_pattern.search(line[1])])
    ...
    types = list(map(get_parameter_type, parameter_type_lines))
...
def try_real_annotations(fn, loc):
    ...
    all_annots = [sig.return_annotation] + [p.annotation for p in list(sig.parameters.values())]
