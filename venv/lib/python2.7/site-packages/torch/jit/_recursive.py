...
def infer_concrete_type_builder(nn_module):
    ...
    for name, item in list(nn_module._parameters.items()):
    ...
    for name, item in list(nn_module._buffers.items()):
        assert item is None or isinstance(item, torch.Tensor)
        attr_type = infer_type(name, item)
        concrete_type_builder.add_attribute(name, attr_type, False)
        added_names.add(name)

    for name, item in list(nn_module._modules.items()):
        attr_type = infer_type(name, item)
        if item is None:
    ...
    for name, ann in list(class_annotations.items()):
    ...
    for name, overloaded_names in list(overloads.items()):
        concrete_type_builder.add_overload(name, overloaded_names)

    for name, value in list(nn_module.__dict__.items()):
    ...
    for name, value in list(type(nn_module).__dict__.items()):
...
def create_script_module_impl(nn_module, concrete_type, stubs_fn):
    ...
    def init_fn(script_module):
        ...
        for name, (attr_type, is_param) in list(concrete_type.get_attributes().items()):
...
def get_overload_name_mapping(overload_info):
    ...
    for orig_fn, overloads in list(overload_info.items()):
...
def infer_methods_to_compile(nn_module):
    ...
    filtered_methods = list(filter(ignore_overloaded, methods))
...
def wrap_cpp_module(cpp_module):
    ...
    """
    def init_fn(script_module):
        for name, cpp_module in list(torch._C.ModuleDict(script_module._c).items()):
...
def lazy_bind(concrete_type, unbound_method):
    ...
    def lazy_binding_method(cpp_module, *args):
        def init_fn(script_module):
            ...
            for name, value in list(concrete_type.get_constants().items()):
