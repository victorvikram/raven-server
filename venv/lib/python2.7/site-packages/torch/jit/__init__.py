...
def save(m, f, _extra_files=DEFAULT_EXTRA_FILES_MAP):
    ...
    if isinstance(f, str) or \
            (sys.version_info[0] == 2 and isinstance(f, str)) or \
...
def _create_interpreter_name_lookup_fn(frames_up=1):
    def _get_interpreter_name_for_var(var):
        ...
        for k, v in list(f_globals.items()):
            if isinstance(v, torch.Tensor) and var is v:
                return k if k != 'self' else ''
        return ''
...
def _check_trace(check_inputs, func, traced_func, check_tolerance,
    ...
    for inputs in check_inputs:
        ...
        if is_trace_module:
            copied_dict = {}
            for name, data in list(inputs.items()):
...
def trace_module(mod,
    ...
    try:
        ...
        for method_name, example_inputs in list(inputs.items()):
...
class OrderedModuleDict(OrderedDictWrapper):
    ...
    def items(self):
        r = list(self._python_modules.items())
        return r
...
class ScriptMeta(type):
    def __init__(cls, name, bases, attrs):
        ...
        for base in reversed(bases):
            for k, v in list(getattr(base, '_methods', {}).items()):
                cls._methods[k] = v
...
if _enabled:
    ...
    class RecursiveScriptModule(ScriptModule):
        ...
        * Scripting inspects the Python code of the model
        ...
        def __setattr__(self, attr, value):
            ...
            elif hasattr(self, "_concrete_type") and attr in list(self._concrete_type.get_constants().keys()):
    ...
    for name, item in list(RecursiveScriptModule.__dict__.items()):
...
class TracedModule(ScriptModule):
    ...
    def __init__(self, orig, id_set=None, _compilation_unit=None):
        ...
        for name, param in list(orig._parameters.items()):
            if param is not None:
                tmp_module._parameters[name] = param
                check_unique(param)
        for name, buf in list(orig._buffers.items()):
            if buf is not None:
                tmp_module._buffers[name] = buf
                check_unique(buf)
        for name, val in list(orig.__dict__.items()):
        ...
        for name, submodule in list(orig._modules.items()):
...
def _check_overload_defaults(impl_defaults, overload_defaults, loc):
    for name, overload_value in list(overload_defaults.items()):
