...
class GradScaler(object):
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def step(self, optimizer, *args, **kwargs):
        ...
        if not sum(v.item() for v in list(optimizer_state["found_inf_per_device"].values())):
    ...
    def update(self, new_scale=None):
        ...
        if new_scale is not None:
            ...
            found_infs = [found_inf.to(device=self._scale.device, non_blocking=True)
                          for state in list(self._per_optimizer_states.values())
                          for found_inf in list(state["found_inf_per_device"].values())]
