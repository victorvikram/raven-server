...
def test_idwtn_missing():
    ...
    for num_missing in range(1, len(coefs)):
        for missing in combinations(list(coefs.keys()), num_missing):
...
def test_dwtn_axes():
    ...
    expected_a = list([pywt.dwt(x, 'haar')[0] for x in data])
    assert_equal(coefs['a'], expected_a)
    expected_d = list([pywt.dwt(x, 'haar')[1] for x in data])
    assert_equal(coefs['d'], expected_d)

    coefs = pywt.dwtn(data, 'haar', axes=(1, 1))
    expected_aa = list([pywt.dwt(x, 'haar')[0] for x in expected_a])
    assert_equal(coefs['aa'], expected_aa)
    expected_ad = list([pywt.dwt(x, 'haar')[1] for x in expected_a])
