...
def swtn(data, wavelet, level, start_level=0, axes=None):
    ...
    if not _have_c99_complex and np.iscomplexobj(data):
        ...
        for rdict, idict in zip(real, imag):
            cplx.append(
                dict((k, rdict[k] + 1j * idict[k]) for k in list(rdict.keys())))
        return cplx
...
def iswtn(coeffs, wavelet, axes=None):
    ...
    ndim_transform = max(len(key) for key in list(coeffs[0].keys()))
    ...
    for j in range(num_levels):
        ...
        common_dtype = np.result_type(*(
            [dt, ] + [v.dtype for v in list(details.values())]))
        if output.dtype != common_dtype:
            output = output.astype(common_dtype)

        # We assume all coefficient arrays are of equal size
        shapes = [v.shape for k, v in list(details.items())]
        if len(set(shapes)) != 1:
            raise RuntimeError(
                "Mismatch in shape of intermediate coefficient arrays")
        ...
        for firsts in product(*([list(range(last_index)), ]*ndim_transform)):
