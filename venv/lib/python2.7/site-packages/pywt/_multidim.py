...
def dwtn(data, wavelet, mode='symmetric', axes=None):
    ...
    if not _have_c99_complex and np.iscomplexobj(data):
        ...
        return dict((k, real[k] + 1j * imag[k]) for k in list(real.keys()))
...
def _fix_coeffs(coeffs):
    missing_keys = [k for k, v in list(coeffs.items()) if v is None]
    if missing_keys:
        raise ValueError(
    ...
    invalid_keys = [k for k, v in list(coeffs.items()) if
                    not set(k) <= set('ad')]
    if invalid_keys:
        raise ValueError(
    ...
    key_lengths = [len(k) for k in list(coeffs.keys())]
    if len(np.unique(key_lengths)) > 1:
        raise ValueError(
            "All detail coefficient names must have equal length.")

    return dict((k, np.asarray(v)) for k, v in list(coeffs.items()))
...
def idwtn(coeffs, wavelet, mode='symmetric', axes=None):
    ...
    coeffs = dict((k, v) for k, v in list(coeffs.items()) if v is not None)

    # Raise error for invalid key combinations
    coeffs = _fix_coeffs(coeffs)

    if (not _have_c99_complex and
            any(np.iscomplexobj(v) for v in list(coeffs.values()))):
        real_coeffs = dict((k, v.real) for k, v in list(coeffs.items()))
        imag_coeffs = dict((k, v.imag) for k, v in list(coeffs.items()))
    ...
    ndim_transform = max(len(key) for key in list(coeffs.keys()))

    try:
        coeff_shapes = (v.shape for k, v in list(coeffs.items())
    ...
    if axes is None:
        axes = list(range(ndim_transform))
        ndim = ndim_transform
    else:
