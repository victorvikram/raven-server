...
COMPRESSION_INFO_REV = {v: k for k, v in list(COMPRESSION_INFO.items())}
...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def named(self):
        ...
        """
        return {TiffTags.lookup(code).name: value for code, value in list(self.items())}
...
for idx, name in list(TYPES.items()):
    name = name.replace(" ", "_")
...
class ImageFileDirectory_v1(ImageFileDirectory_v2):
    ...
    def from_v2(cls, original):
        ...
        ifd.next = original.__next__  # an indicator for multipage tiffs
        return ifd
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        self.__first = self.__next = self.tag_v2.__next__
        self.__frame = -1
        self.__fp = self.fp
        self._frame_pos = []
        self._n_frames = None
    ...
    def _seek(self, frame):
        ...
        while len(self._frame_pos) <= frame:
            ...
            self.__next = self.tag_v2.__next__
            if self.__next == 0:
...
def _save(im, fp, filename):
    ...
    if libtiff:
        ...
        for tag, value in itertools.chain(
            list(ifd.items()), list(getattr(im, "tag_v2", {}).items()), list(legacy_ifd.items())
        ):
            ...
            if tag not in TiffTags.LIBTIFF_CORE:
                ...
                if tag in ifd.tagtype:
                    ...
                    or (not py3 and isinstance(value, str))  # noqa: F821
                ):
                    continue
            if tag not in atts and tag not in blocklist:
                if isinstance(value, str if py3 else str):  # noqa: F821
