    ...
    for size, name in data:
        ...
        for c in name:
            ...
            if len(curr.name) > max_depth:
                break
    symbol_trie.size = sum(t.size for t in list(symbol_trie.dictionary.values()))
    return symbol_trie
...
def PrintTrie(trie, prefix, max_depth, min_size, color):
    ...
    """
    if len(trie.name) == max_depth or not list(trie.dictionary.keys()):
        ...
        print('{0}{1} {2}'.format(
              ...
              ReadableSize(trie.size)))
        keys_with_sizes = [
            (k, trie.dictionary[k].size) for k in list(trie.dictionary.keys())]
