...
if __name__ == '__main__':
    ...
    G.remove_edge(0, 1)
    G.add_edges_from(list(zip(list(range(0, 3)), list(range(1, 4)))), weight=10)
    print(("Edges in G: ", G.edges(data=True)))
    G.remove_edges_from(list(zip(list(range(0, 3)), list(range(1, 4)))))
    print(("Edges in G: ", G.edges(data=True)))

    G = PrintGraph()
    nx.add_path(G, list(range(10)))
    nx.add_star(G, list(range(9, 13)))
    nx.draw(G)
    plt.show()
